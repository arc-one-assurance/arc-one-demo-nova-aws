#!/usr/bin/env python3
"""
Canonical registration script — copy into agent repos as scripts/register_arc_one_manifest.py.

Kept in sync with apps/api/arc_one_api/cli/agent_manifest.py (MVP + v2 mapping).
"""
...
def main() -> None:
    ...
    args = ap.parse_args()
    base = os.environ.get("ARC_ONE_API_BASE_URL", "http://127.0.0.1:8000")
    token = os.environ.get("ARC_ONE_BEARER_TOKEN", "")
    debug_sub = os.environ.get("ARC_ONE_DEBUG_SUB", "")
    apply(args.manifest, base_url=base, dry_run=args.dry_run, token=token, debug_sub=debug_sub)
